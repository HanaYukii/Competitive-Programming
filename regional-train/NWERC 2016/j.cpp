...
int main(){
    ...
    f1(n){
        ...
        fr(j,1,a+1){
            int b = n*2 * (j-1) + i*2-1;
            dinic.add_edge(0,b,cnt[j]);
            dinic.add_edge(b,b+1,c[j]);
            dinic.add_edge(b+1,9000+i,INF);
            if(i!=n){
                dinic.add_edge(b+1,b+2,INF);
            }
        }
        dinic.add_edge(9000+i,9500,d);
    }
    if(dinic.maxflow(0,9500)==sum){
        cout<<"possible"<<'\n';
    }
    else{
        cout<<"impossible"<<'\n';
